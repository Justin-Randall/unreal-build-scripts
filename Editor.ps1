...
if (-not (Test-Path $ProjectPath)) { Write-Error "Project file not found at '$ProjectPath'"; exit 1 }
...
# Define target
# -------------------------------------------------------------------
$EditorTarget = "${ProjectName}Editor"  # e.g. "WillCraftDemoEditor"

$logDir = Join-Path $ProjectDir "Intermediate\BuildLogs\Editor"
if (-not (Test-Path $logDir)) {
	New-Item -ItemType Directory -Path $logDir | Out-Null
}
$logFile = Join-Path $logDir "$EditorTarget-$Configuration-$Platform.log"
...
Write-Host "→ Building '$EditorTarget' ($Platform / $Configuration) via dotnet UBT..."
